...
def map_sites_to_enzymes(df, trie, pssms, code_to_name):
    ...
    for row in df.itertuples(index=True, name="Row"):
        ...
        n_term_match = code_to_name[find_best_match(trie.match(n_term_window), pssms, n_term_window)]
        c_term_match = code_to_name[find_best_match(trie.match(c_term_window), pssms, c_term_window)]
        new_rows = []
    ...
    for protein in per_protein_results:
        ...
        print(per_protein_counts[protein].keys())
...
def find_best_match(matches, pssms, cleavage_site):
    ...
    return best_match if best_match is not None else "unspecified cleavage"
